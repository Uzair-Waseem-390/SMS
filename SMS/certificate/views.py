...
#             if form.is_valid():
#                 # Generate PDF and save certificate
...
#                 # Render HTML
#                 html_content = _render_template_body(template.body_template, ctx)

#                 serial = _generate_serial_number(branch)
#                 ctx['serial_number'] = serial

#                 # Generate PDF
#                 pdf_content, pdf_error = _html_to_pdf(html_content, school, branch, template, ctx)
#                 if pdf_content is None:
#                     messages.error(request, pdf_error or 'Failed to generate PDF.')
...
#                     custom_data=ctx,
#                 )
#                 safe_name = "".join(c if c.isalnum() or c in ' _-' else '_' for c in (ctx.get('recipient_name') or 'cert'))
#                 fname = f"cert_{safe_name}_{timezone.now().strftime('%Y%m%d%H')}.pdf"
#                 cert.generated_pdf.save(fname, pdf_content, save=True)
#                 for k in ['cert_gen_step', 'cert_gen_template_pk', 'cert_gen_student_pk', 'cert_gen_employee_id']:
#                     request.session.pop(k, None)
...
# def _html_to_pdf(html_content, school, branch, template, ctx):
#     """Convert HTML to PDF using WeasyPrint. Returns (ContentFile, error_message).
#     If successful, error_message is None. If failed, ContentFile is None and error_message is set.
#     """
#     try:
#         from weasyprint import HTML
#         from django.core.files.base import ContentFile
#     except ImportError as e:
#         return None, "WeasyPrint is not installed. Install it with: pip install weasyprint"
...
# .cert-header {{ text-align: center; margin-bottom: 20px; font-size: 18pt; font-weight: bold; }}
# .cert-body {{ text-align: justify; margin: 20px 0; }}
# .cert-footer {{ margin-top: 30px; text-align: right; font-size: 10pt; }}
# </style>
# </head>
# <body>
# <div class="cert-header">{school.name}</div>
# <div class="cert-header" style="font-size:14pt;">{branch.name}</div>
# <div class="cert-body">{html_content}</div>
# <div class="cert-footer">Serial: {ctx.get('serial_number', '')} | Date: {ctx.get('date', '')}</div>
# </body>
# </html>
# """
#     try:
#         pdf_bytes = HTML(string=full_html).write_pdf()
#         return ContentFile(pdf_bytes), None
#     except Exception as e:
#         err = str(e)
#         if "cairo" in err.lower() or "gdk" in err.lower() or "pango" in err.lower() or "library" in err.lower():
#             return None, (
#                 "WeasyPrint could not load a system library (GTK/Cairo/Pango). "
#                 "On Windows, install GTK3 runtime: https://github.com/tschoonj/GTK-for-Windows-Runtime-Environment-Installer/releases "
#                 "Or try: pip install weasyprint (with a supported environment). Error: " + err[:200]
#             )
#         return None, f"PDF generation failed: {err[:300]}"
...
#             as_attachment=True,
#             filename=f"certificate_{cert.serial_number}.pdf",
#         )
#     except Exception:
#         raise Http404
